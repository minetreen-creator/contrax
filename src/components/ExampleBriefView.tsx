...
/**
 * ExampleBriefView — the PURE presentation half of the example-brief surface.
 *
 * Split out of src/components/ExampleBrief.tsx (owner order 2026-09-18) for two
 * reasons:
 *   1. It takes the loaded state as a PROP, so the render rules are unit
 *      testable without a browser, a server fn or a DB.
 *   2. It makes the honest-empty behavior explicit and impossible to regress:
 *        - variant="embed" (HOMEPAGE) with NO valid example → renders NOTHING
 *          (the whole section disappears; the owner banned "No example brief is
 *          available right now." on the homepage).
 *        - variant="page" (standalone /example-brief) with none → keeps the
 *          honest fallback + a link back, because there must be *somewhere* an
 *          empty state is allowed to speak.
 *
 * One source of truth for BOTH surfaces: same data, same labels, same evidence
 * card — only the empty behavior differs.
 */
export function ExampleBriefView({
  brief,
  variant = "page",
  onRadarClick,
}: {
  brief: ExampleBriefData | null | undefined;
  variant?: "page" | "embed";
  onRadarClick?: () => void;
}) {
  /** Real value or "Value not disclosed" — never "Not specified estimated". */
  const placeholderValues = /^(not specified|unknown|n\/?a|none|tbd|to be determined)$/i;
  const realValue = (() => {
    const s = String(brief?.estimated_value ?? "").trim();
    if (!s || placeholderValues.test(s)) return null;
    return s;
  })();

  // HOMEPAGE: no valid example → hide the entire section (owner order).
  if (variant === "embed" && brief === null) return null;
        ...
        <article className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900">
          <div className="border-b border-slate-800 px-5 py-4">
            ...
            <p className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-300">
              ...
              <span aria-hidden="true">·</span>
              <span>
                {realValue ? `${realValue} estimated` : "Value not disclosed"}
              </span>
              {due && (
                <>
                  <span aria-hidden="true">·</span>
...
export default ExampleBriefView;
