...
import { NO_SOURCE_BADGE, sourceBadgeLabel, sourceBadgeTone, type SourceBadgeTone } from "~/lib/cert-matching";
import { TrialGate } from "~/components/TrialGate";

/**
 * Provenance-badge colours per source class (PR-1 restructure). The badge TEXT
 * comes from `sourceBadgeLabel` (Federal / State (PA) / the city's name), and an
 * INTERNAL source yields NO badge at all — so this map only ever colours a real
 * jurisdiction claim.
 */
const SOURCE_BADGE_CLASS: Record<SourceBadgeTone, string> = {
...
const getAlerts = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const rows = await sql()`SELECT a.id, a.bid_id, a.is_read, a.created_at, b.title, b.agency, b.due_date, b.category, b.set_aside, b.source_url, b.source FROM bid_alerts a JOIN bids b ON b.id=a.bid_id WHERE a.user_id=${user.id} ORDER BY a.created_at DESC LIMIT 50`;
