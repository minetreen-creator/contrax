  ...
  test("coverage reads the ladder honestly on a schema.sql-only database", async () => {
    await onBootstrapDatabase(async () => {
      ...
      expect(payload.headline).toBe(
        "State grant coverage: 14 of 51 states have a validated source (0 connected, 0 curated, 14 limited)",
      );
      expect(payload.counts).toEqual({ total: 51, validated: 14, connected: 0, curated: 0, limited: 14, unavailable: 37 });
