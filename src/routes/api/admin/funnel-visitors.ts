...
async function handler({ request }: { request: Request }) {
  ...
  // Empty-fragment when source is null; `AND source = '<value>'` when active.
  // Added to the WHERE of every query so filtering is eager, not post-hoc.
  // Built as a RAW fragment via sql().unsafe(), matching the codebase's
  // established admin pattern (see src/routes/api/admin/acquisition.ts `notBot`):
  // `sql` from ~/db is the neon connection FACTORY — tagging it bare (or tagging
  // a `sql()` client) EXECUTES a query instead of producing a fragment, so it
  // can never be interpolated here. The value is single-quote escaped, keeping
  // the WHERE text semantically identical to a bound parameter.
  const sourceFilter = sql().unsafe(
    source === null ? "" : `AND source = '${source.replace(/'/g, "''")}'`,
  );
