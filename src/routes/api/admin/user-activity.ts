 ...
 *
 * Returns 400/403 on auth/admin failure, 500 on any query failure (clean JSON
...
async function handler({ request }: { request: Request }) {
  ...
  try {
    ...
    // then merged onto the user rows in JS by numeric user id.
    const [loginRows, searchRows, scoreRows, saveRows] = await Promise.all([
      sql()`
