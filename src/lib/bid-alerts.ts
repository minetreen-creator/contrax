...
/** Create one alert per user/new bid. Safe to call repeatedly (unique constraint deduplicates). */
export async function generateBidAlerts(bidIds: number[]): Promise<number> {
  ...
  for (const bidId of bidIds) {
    const rows = await sql()`SELECT id, title, agency, category, set_aside, description FROM bids WHERE id = ${bidId}`;
