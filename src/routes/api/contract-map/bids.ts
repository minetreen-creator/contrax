...
async function handler({ request }: { request: Request }) {
  ...
  if (raw !== UNSPECIFIED && (!/^[A-Z]{2}$/.test(raw) || !US_STATES.includes(raw as any))) {
    return Response.json({ error: "Unknown state code" }, { status: 400 });
  }
  const isUnspecified = raw === UNSPECIFIED;
  const state = isUnspecified ? UNSPECIFIED : raw;
  try {
    ...
    const bids = (rows as any[])
      .filter((r) =>
        // Unspecified = bids we cannot honestly attribute to a state; equals
        // totals.unspecified in /api/contract-map. We never fabricate a state.
        isUnspecified ? deriveStateCode(r.location) === null : deriveStateCode(r.location) === state,
      )
      .map((r) => ({
        id: r.id,
        title: r.title,
    ...
    return Response.json({
      state,
      name: isUnspecified ? "Nationwide / Location not specified" : STATE_NAMES[state],
      bids,
    });
  } catch {
