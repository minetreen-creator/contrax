 ...
 * authenticated account. Runs inside a server fn so `getRequest()` (the only
 * way to read the request cookie in this route) is in a proper server scope.
 * Best-effort / fail-open — never affects the OAuth result.
 */
...
const handleGoogleAuth = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { code, plan: _plan } }) => {
    ...
    if (userId === null) {
      ...
      // begins when the user upgrades to a paid plan — the hard Pro gate returns
      // GATE_REQUIRED on gated attempts, so this no longer starts lazily.
