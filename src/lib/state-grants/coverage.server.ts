...
/**
 * "State grant coverage: 1 of 51 states have a validated source (0 connected, 0
 * curated, 1 limited)." — the noun agrees with the TOTAL (51 states). The
 * sentence never claims coverage the ladder has not granted.
 */
export function coverageHeadline(counts: CoverageCounts): string {
  const noun = counts.total === 1 ? "state has" : "states have";
  return (
    `State grant coverage: ${counts.validated} of ${counts.total} ${noun} a validated source ` +
