/**
 * South Carolina Procurement Source
 *
 * Uses SAM.gov API filtered by "South Carolina" keyword to find
 * SC-specific contract opportunities for small businesses.
 *
 * API: GET https://sam.gov/api/prod/sgs/v1/search/?q=South+Carolina&...
 */
...
export async function fetchBids(): Promise<RawBid[]> {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    try {
      const url = `${SAM_API}?page=${page}&size=${PAGE_SIZE}&sort=-modifiedDate&mode=opportunities&q=South+Carolina&is_active=true`;
      console.log(`  SC: fetching page ${page + 1}/${MAX_PAGES}...`);

      const resp = await fetch(url, { headers: HEADERS });
      if (!resp.ok) {
        console.error(`  SC page ${page} returned ${resp.status}`);
        if (page > 0) break;
        continue;
      }
      ...
      for (const item of items) {
        try {
          ...
          const agency =
            deepestOrg?.name || orgs[0]?.name || "South Carolina Agency";
          ...
          results.push({
            external_id: `sc-${item._id || item.solicitationNumber || `page${page}-${results.length}`}`,
          ...
          console.error(`  SC: error parsing item:`, (e as Error).message);
        }
      }

      console.log(`  SC page ${page + 1}: got ${items.length} items (total: ${results.length})`);

      if (items.length < PAGE_SIZE) break;
      await new Promise((r) => setTimeout(r, DELAY_MS));
    } catch (e) {
      console.error(`  SC page ${page} error:`, (e as Error).message);
    }
  }
