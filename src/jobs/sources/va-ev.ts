/**
 * Virginia eVA / State Procurement Source
 *
 * Since eVA (eva.virginia.gov) uses a session-based JSP vendor portal that is
 * not accessible via simple HTTP fetch, we use SAM.gov's API filtered for
 * Virginia-specific opportunities as a practical state-level data source.
 *
 * This queries SAM.gov for active opportunities mentioning "Virginia" in the
 * search and tags them as source='va_evirginia' so they appear as a separate
 * feed from the main SAM.gov source.
 *
 * API: GET https://sam.gov/api/prod/sgs/v1/search/?q=Virginia&...
...
const PAGE_SIZE = 25;
const MAX_PAGES = 2; // 50 bids max for state-level
const DELAY_MS = 500;
...
export async function fetchBids(): Promise<RawBid[]> {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    try {
      const url = `${SAM_API}?page=${page}&size=${PAGE_SIZE}&sort=-modifiedDate&mode=opportunities&q=Virginia&is_active=true`;
      console.log(`  VA eVA: fetching page ${page + 1}/${MAX_PAGES}...`);

      const resp = await fetch(url, { headers: HEADERS });
      if (!resp.ok) {
        console.error(`  VA eVA page ${page} returned ${resp.status}`);
        if (page > 0) break;
        continue;
      }
      ...
      for (const item of items) {
        try {
          ...
          const agency =
            deepestOrg?.name || orgs[0]?.name || "Virginia Agency";
          ...
          console.error(`  VA eVA: error parsing item:`, (e as Error).message);
        }
      }

      console.log(`  VA eVA page ${page + 1}: got ${items.length} items (total: ${results.length})`);

      if (items.length < PAGE_SIZE) break;
      await new Promise((r) => setTimeout(r, DELAY_MS));
    } catch (e) {
      console.error(`  VA eVA page ${page} error:`, (e as Error).message);
    }
  }
