...
/**
 * GET /api/state-grants/coverage — honest state-grant coverage facts (owner
 * ROLLOUT order 2026-09-18, part 2; re-cut onto the corrected model, R1
 * 2026-09-19).
 *
 ...
 * the mirror can never change what coverage Contrax claims. The tiers are the
 * owner's ladder (connected | curated | limited | unavailable), and Virginia is
 * `limited` with its honesty note attached. The sync half (last successful sync,
 * record counts, per-status counts) comes from the live tables.
 *
 * RESPONSE 200 { ok, headline, noNationwideCoverage, counts, ladder, states[],
 *                validated[], asOf, notice, federalGrantsUrl, searchUrl,
 *                coverageUrl }
