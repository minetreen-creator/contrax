...
export function NaicsTypeahead({
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {value.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-2">
          {value.map((code) => (
            <span
              key={code}
              className="inline-flex max-w-full items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 py-1 pl-2 pr-1"
            >
              <span className="font-mono text-xs font-bold text-blue-700">{code}</span>
              <span className="max-w-[12rem] truncate text-xs font-medium text-slate-700 sm:max-w-[20rem]">
                {naicsTitle(code)}
              </span>
              <button
                type="button"
                ...
                className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-slate-400 transition hover:bg-blue-100 hover:text-slate-700"
              >
      ...
      {open && term.trim() && (
        <ul className="absolute z-20 mt-1 max-h-72 w-full overflow-auto rounded-lg border border-slate-200 bg-white py-1 shadow-xl">
            ...
            return (
              <li key={s.code}>
                <button
                  ...
                  disabled={disabled}
                  onClick={() => add(s.code)}
                  className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
