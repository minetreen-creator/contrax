/**
 * Competitive Pricing Engine — AI-powered bid price suggestions derived from
 * analyzing past awarded contracts for similar work.
 */
...
/** Load all cached pricing recommendations for the current user. */
export const fetchPricingCache = createServerFn({ method: "GET" }).handler(async (): Promise<PricingRecommendation[]> => {
  ...
  try {
    ...
    return (rows as any[]).map((r: any) => ({
      bid_id: String(r.bid_id), bid_title: r.bid_title || "",
      suggested_low: Number(r.suggested_low), suggested_high: Number(r.suggested_high), suggested_median: Number(r.suggested_median),
      confidence: Number(r.confidence), comparable_awards: Array.isArray(r.comparable_awards) ? r.comparable_awards : [],
      rationale: r.rationale || "", pricing_strategy: r.pricing_strategy || "competitive", created_at: String(r.created_at),
    }));
  } catch { return []; }
});

export const getPricingRecommendation = createServerFn({ method: "POST" })
  .validator((data: unknown) => {
    const d = data as any;
    if (!d || typeof d.bid_title !== "string" || typeof d.bid_id !== "string") {
      throw new Error("Invalid pricing input — bid_title and bid_id are required");
    }
    return d as {
      bid_id: string;
      bid_title: string;
...
- Dollar amounts should be realistic given the comparable awards and the estimated value`;
    ...
    try {
      ...
      throw new Error(`Pricing analysis failed: ${msg}`);
    }
  });
