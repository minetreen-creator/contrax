 ...
 *   1. nonprofit_applications — one application per user (UNIQUE), the owner's signup
 *      fields + the audit/evidence columns every verification decision must carry.
    ...
    (SELECT count(*)::int FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'nonprofit_applications'
         AND column_name IN ('submitted_name_normalized', 'matched_bmf_name', 'bmf_status',
             ...
             'reverify_due_at', 'decision', 'decision_flags')) AS audit_columns,
    (SELECT count(*)::int FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'nonprofit_applications'
...
if (failed > 0) {
  ...
  Number(shape[0]?.audit_columns ?? 0) !== 15 ||
